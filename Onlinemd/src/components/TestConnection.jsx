...
const TestConnection = () => {
  ...
  const testBackendConnection = async () => {
    ...
    try {
      ...
      setTestResult(`✅ Connection successful! Found ${response.data.length || 0} users.`);
    } catch (error) {
      console.error('Connection test error:', error);
      if (error.response) {
        setTestResult(`❌ Connection failed: ${error.response.status} - ${error.response.data?.message || error.response.statusText}`);
      } else {
        setTestResult(`❌ Connection error: ${error.message}`);
      }
    } finally {
